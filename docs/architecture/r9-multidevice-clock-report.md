...
尚未执行 R9 两小时实机播放。当前 `adb devices -l` 结果为空，不能沿用 R7/R8 的旧设备在线记录。
...
R9 实现与自动门禁通过；两小时实机与人工音频验收待完成，**暂不宣布 R9 全阶段通过**。保持独立分支，等待设备与用户验收，不自动进入后续架构门禁、R10 或 Release Pipeline。
